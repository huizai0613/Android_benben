...
public class ImageProvider {
	...
	public ImageProvider(Context context, ImageMemoryCache memoryCache,
	 ...
	 * @return
	 */
	public BitmapDrawable createBitmapDrawable(Resources resources,
			Bitmap bitmap) {
		if (bitmap == null) {
			return null;
		}
	 ...
	 */
	public Bitmap fetchBitmapData(ImageLoader imageLoader, ImageTask imageTask,
		...
		if (mDiskCacheProvider != null) {
			...
			if (DEBUG) {
				...
				Log.d(TAG, String.format(MSG_FETCH_BEGIN_IDENTITY_KEY,
						imageTask, imageTask.getIdentityKey()));
				Log.d(TAG, String.format(MSG_FETCH_BEGIN_FILE_CACHE_KEY,
						imageTask, fileCacheKey));
				Log.d(TAG, String.format(MSG_FETCH_BEGIN_ORIGIN_URL, imageTask,
						imageTask.getOriginUrl()));
				Log.d(TAG, String.format(MSG_FETCH_BEGIN_IDENTITY_URL,
						imageTask, imageTask.getIdentityUrl()));
			}
			...
			// try to reuse
			if (inputStream == null) {
				if (reuseInfo != null && reuseInfo.getReuseSizeList() != null
						&& reuseInfo.getReuseSizeList().length > 0) {
					if (DEBUG) {
						Log.d(TAG,
								String.format(MSG_FETCH_TRY_REUSE, imageTask));
					}

					final String[] sizeKeyList = reuseInfo.getReuseSizeList();
					for (int i = 0; i < sizeKeyList.length; i++) {
						String size = sizeKeyList[i];
						final String key = imageTask
								.generateFileCacheKeyForReuse(size);
						inputStream = mDiskCacheProvider.getInputStream(key);

						if (inputStream != null) {
							if (DEBUG) {
								Log.d(TAG, String.format(
										MSG_FETCH_REUSE_SUCCESS, imageTask,
										size));
							}
							break;
						} else {
							if (DEBUG) {
								Log.d(TAG, String.format(MSG_FETCH_REUSE_FAIL,
										imageTask, size, key));
							}
						}
					}
				...
				if (DEBUG) {
					Log.d(TAG,
							String.format(MSG_FETCH_HIT_DISK_CACHE, imageTask));
				}
			}
			...
			if (inputStream == null) {
				...
				if (DEBUG) {
					Log.d(TAG,
							String.format(MSG_FETCH_DOWNLOAD, imageTask, url));
				}
				inputStream = mDiskCacheProvider.downloadAndGetInputStream(
						imageLoader.getImageDownloader(), imageTask,
						fileCacheKey, url);
				if (imageTask.getStatistics() != null) {
				...
				if (inputStream == null) {
					...
					CLog.e(TAG, "%s download fail: %s %s", imageTask,
							fileCacheKey, url);
				}
			}
			if (inputStream != null) {
				try {
					bitmap = decodeSampledBitmapFromDescriptor(
							inputStream.getFD(), imageTask, imageResizer);
					if (bitmap == null) {
						imageTask.setError(ImageTask.ERROR_BAD_FORMAT);
						CLog.e(TAG,
								"%s decode bitmap fail, bad format. %s, %s",
								imageTask, fileCacheKey,
								imageResizer.getRemoteUrl(imageTask));
					}
				} catch (IOException e) {
					CLog.e(TAG,
							"%s decode bitmap fail, may be out of memory. %s, %s",
							imageTask, fileCacheKey,
							imageResizer.getRemoteUrl(imageTask));
					e.printStackTrace();
				}
			} else {
				CLog.e(TAG, "%s fetch bitmap fail. %s, %s", imageTask,
						fileCacheKey, imageResizer.getRemoteUrl(imageTask));
			}
		}
		return bitmap;
	}
	...
	private Bitmap decodeSampledBitmapFromDescriptor(
			FileDescriptor fileDescriptor, ImageTask imageTask,
			ImageResizer imageResizer) {
		...
		if (DEBUG) {
			Log.d(TAG, String.format(MSG_DECODE, imageTask,
					imageTask.getBitmapOriginSize().x,
	...
	private Bitmap decodeSampledBitmapFromInputStream(InputStream stream,
			ImageTask imageTask, ImageResizer imageResizer) {
		// First decode with inJustDecodeBounds=true to check dimensions
		final BitmapFactory.Options options = new BitmapFactory.Options();
		options.inJustDecodeBounds = true;
	...
	public String getFileCachePath() {
		if (null != mDiskCacheProvider) {
			return mDiskCacheProvider.getDiskCache().getDirectory()
					.getAbsolutePath();
		}
		return null;
	}
	...
	public long getFileCacheUsedSpace() {
		return null != mDiskCacheProvider ? mDiskCacheProvider.getDiskCache()
				.getSize() : 0;
	}
